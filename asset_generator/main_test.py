...
if len(sys.argv) == 1:
    ...
    BASE_DATA_PATH = INPUT_PATH + "/base/SSY_Live_3.7.8_221103.xlsx"
    PLANNING_DATA_PATH = INPUT_PATH + "/planning_1110_sep_test.xlsx"
    MAPPING_DATA_PATH = INPUT_PATH + "/mapping.xlsx"
...
planning_df = pd.read_excel(PLANNING_DATA_PATH, sheet_name=None)

all_planning = {}

for key, df in planning_df.items():
    planning_temp = json.loads(utils.convert_to_json(df))
    all_planning[key] = planning_temp
...
for planning_sheet_name, planning_dict in all_planning.items():
    ...
    print("===============================")
    print("Start Making [ " + planning_sheet_name + " ] in planning")
    ...
    popup_store_df = packaging.make_df_popup_store(
        popup_store_df, popup_list, popup_locale_dict, image_url, update_date
    )
    ...
    for bg_dict in input_dict["bgList"]:
        if "group" in bg_dict:
            ...
            bg_dict["group"] = group_list[0].origin_group_name
            bg_dict["groupID"] = group_list[0].origin_group_id
        if "groupID" not in bg_dict:
            print("배경에 해당하는 그룹이 없습니다.")
            exit()

        if "theme" not in bg_dict:
            bg_dict["theme"] = [theme_list[0].theme_name, theme_list[0].image_theme]
    ...
    for event_dict in input_dict["eventList"]:
        if "group" in event_dict:
            ...
            event_dict["group"] = group_list[0].origin_group_name
            event_dict["eventGroupCode"] = group_list[0].origin_group_id
        if "eventGroupCode" not in event_dict:
            print("미션 이벤트에 해당하는 그룹이 없습니다.")
            exit()

        if "theme" not in event_dict:
            event_dict["theme"] = [theme_list[0].theme_name, theme_list[0].image_theme]
                        ...
                        for menu in input_dict["menuList"]:
                            if menu["name"][0] == mission_detail_dict["menu"]:
                                if "newcode" in menu:
                                    ...
                                    print(
                                        "[ "
                                        + event_dict["eventNameLocale"][0]
                                        + " ] 에서 [ "
                                        + mission_detail_dict["menu"]
                                        + " ] 의 EventMissionData > uri 를 생성하지 못하였습니다."
                                    )
